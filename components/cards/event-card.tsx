...
export function EventCard({ event, delay }: EventCardProps) {
  ...
  return (
    <Reveal delay={delay} className="h-full">
      <article className="motion-card flex h-full min-h-[26rem] min-w-0 flex-col gap-6 rounded-2xl border border-white/10 bg-zinc-950/80 p-5 shadow-[0_18px_70px_rgba(0,0,0,0.28)] transition duration-300 hover:-translate-y-1 hover:border-blood/60 hover:shadow-[0_24px_90px_rgba(193,18,31,0.14)] sm:p-6">
        <div className="flex min-h-20 items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <h3 className="mt-3 break-words text-2xl font-black leading-tight text-white">{event.title}</h3>
          </div>
        </div>
        <p className="min-h-16 break-words text-sm font-medium leading-6 text-zinc-300">{event.description}</p>
