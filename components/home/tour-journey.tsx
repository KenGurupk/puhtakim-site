...
function TourStopCard({ event, index }: { event: TourStop; index: number }) {
  ...
  return (
    <motion.article
      ...
      initial={false}
      animate={{ y: isInView ? -6 : 0, scale: 1 }}
      whileHover={{ y: -10, rotateX: 0.45, rotateY: -0.45 }}
    ...
    >
      ...
      <p className="break-words text-xs font-black tracking-[0.08em] text-blood">{event.stop}</p>
      ...
      <div className="mt-5 inline-flex min-h-12 w-full items-center justify-center rounded-2xl border border-emerald-300/30 bg-emerald-300/10 px-5 py-3 text-center text-sm font-black text-emerald-200">
        הסתיים בהצלחה ✓
      </div>
    </motion.article>
  );
