...
function TourStopCard({ event, index }: { event: TourStop; index: number }) {
  ...
  return (
    <motion.article
      ref={ref}
      className={`motion-card group relative z-10 flex h-full min-h-[34rem] flex-col overflow-hidden rounded-2xl border bg-black/78 p-6 shadow-[0_18px_70px_rgba(0,0,0,0.38)] backdrop-blur-sm transition-colors duration-300 ${
        isInView
    ...
    >
      ...
      <p className="text-xs font-black tracking-[0.08em] text-blood">
        {event.stop}
      </p>
      <div className="mt-8 space-y-5">
        <div className="min-h-[5.25rem]">
          <p className="text-xs font-black tracking-[0.08em] text-zinc-500">עיר</p>
          <h3 className="mt-2 text-4xl font-black leading-tight text-white">{event.city}</h3>
        </div>
        <div className="min-h-[4.5rem]">
          <p className="text-xs font-black tracking-[0.08em] text-zinc-500">אולם</p>
          <p className="mt-2 text-2xl font-black text-white/88">{event.venue}</p>
        </div>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="min-h-[4.25rem]">
            ...
            {event.time && <p className="mt-2 text-sm font-black leading-6 text-blood" dir="ltr">{scheduleRows[0]?.time}</p>}
          </div>
        </div>
        <div className="min-h-[4.75rem]">
          <p className="text-xs font-black tracking-[0.08em] text-zinc-500">כתובת</p>
          <p className="mt-2 text-sm font-bold leading-6 text-zinc-400">{event.address}</p>
        </div>
        <div className="min-h-[13.25rem]">
          ...
          <div className="mt-3 grid gap-2">
            {scheduleRows.map((row) => (
              ...
              >
                <p className="text-sm font-black leading-5 text-blood" dir="ltr">{row.time}</p>
                <p className="mt-1 text-sm font-black leading-6 text-white">{row.activity}</p>
              </div>
            ))}
          </div>
        </div>
...
export function TourJourney({ stops }: TourJourneyProps) {
  ...
  return (
    <div ref={ref} className="relative mt-16 grid gap-7 sm:grid-cols-2 lg:grid-cols-4 lg:gap-5" style={{ position: "relative" }}>
